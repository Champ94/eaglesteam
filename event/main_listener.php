...
class main_listener implements EventSubscriberInterface
{
	...
	public function add_page_header_link()
	{
		$this->template->assign_vars(array(
			'U_DEMO_PAGE'	=> $this->helper->route('champ94_eaglesteam_controller', array('name' => 'world')),
		));
	}
	...
	public function viewonline_page($event)
	{
		if ($event['on_page'][1] === 'app' && strrpos($event['row']['session_page'], 'app.' . $this->php_ext . '/demo') === 0)
		{
			...
			$event['location_url'] = $this->helper->route('champ94_eaglesteam_controller', array('name' => 'world'));
		}
	}
